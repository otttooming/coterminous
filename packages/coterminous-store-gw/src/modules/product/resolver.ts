...
const wp = new WPAPI({ endpoint: 'https://dev-spiceflow.pantheonsite.io/wp-json' });
...
async function getEdges(props) {
  const anyt = await wp.products().param('consumer_key', 'ck_a67345603373570cfc213659c53f941e2dc8c64f').param('consumer_secret', 'cs_186c4d5992dab11ceb56059bccf0dc31ea77c37b');
  ...
  return anyt.map((node) => {
    ...
    return {
      node: {
        name: node.name,
        createdAt: new Date(node.date_created_gmt).getTime(),
      }
    };
  });
}
